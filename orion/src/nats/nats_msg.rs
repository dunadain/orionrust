use bytes::{Buf, BufMut, Bytes, BytesMut};

pub fn encode(id: u32, proto_id: u16, uid: String, svrid: u32, data: Bytes) -> Bytes {
    let mut buf = BytesMut::with_capacity(4 + 2 + uid.len() + 4 + data.len());
    buf.put_u32(id);
    buf.put_u16(proto_id);
    buf.put_u8(uid.len() as u8);
    buf.put_slice(uid.as_bytes());
    buf.put_u32(svrid);
    buf.put(data);
    buf.freeze()
}

pub fn decode(mut bytes: Bytes) -> (u32, u16, String, u32, Bytes) {
    let id = bytes.get_u32();
    let proto_id = bytes.get_u16();
    let uid_len = bytes.get_u8();
    let uid = String::from_utf8(bytes.slice(0..uid_len as usize).to_vec()).unwrap();
    bytes.advance(uid_len as usize);
    let svrid = bytes.get_u32();
    (id, proto_id, uid, svrid, bytes)
}
#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_encode_decode() {
        let id = 123;
        let proto_id = 456;
        let uid = String::from("test");
        let svrid = 789;
        let data = Bytes::from("payload");

        let encoded = encode(id, proto_id, uid.clone(), svrid, data.clone());
        let (decoded_id, decoded_proto_id, decoded_uid, decoded_svrid, decoded_data) =
            decode(encoded);

        assert_eq!(decoded_id, id);
        assert_eq!(decoded_proto_id, proto_id);
        assert_eq!(decoded_uid, uid);
        assert_eq!(decoded_svrid, svrid);
        assert_eq!(decoded_data, data);
    }

    #[test]
    fn test_encode_decode_empty_data() {
        let id = 123;
        let proto_id = 456;
        let uid = String::from("test");
        let svrid = 789;
        let data = Bytes::new();

        let encoded = encode(id, proto_id, uid.clone(), svrid, data.clone());
        let (decoded_id, decoded_proto_id, decoded_uid, decoded_svrid, decoded_data) =
            decode(encoded);

        assert_eq!(decoded_id, id);
        assert_eq!(decoded_proto_id, proto_id);
        assert_eq!(decoded_uid, uid);
        assert_eq!(decoded_svrid, svrid);
        assert_eq!(decoded_data, data);
    }

    #[test]
    fn test_encode_decode_long_uid() {
        let id = 123;
        let proto_id = 456;
        let uid = String::from("this_is_a_very_long_uid_that_exceeds_the_capacity_of_a_u8");
        let svrid = 789;
        let data = Bytes::from("payload");

        let encoded = encode(id, proto_id, uid.clone(), svrid, data.clone());
        let (decoded_id, decoded_proto_id, decoded_uid, decoded_svrid, decoded_data) =
            decode(encoded);

        assert_eq!(decoded_id, id);
        assert_eq!(decoded_proto_id, proto_id);
        assert_eq!(decoded_uid, uid);
        assert_eq!(decoded_svrid, svrid);
        assert_eq!(decoded_data, data);
    }
}
